...
const esquemaUsuarioDB=new mongoose.Schema({
        ...
        nomUsuario:{type:String,required:true,unique:true},
...
export class UsuariosCMDB{
    ...
    static async postUsuario(nuevoUsuario){
        ...
        try{  
            const ultimoUsuario=await UsuarioDB.findOne().sort({idUsuario:-1});
            console.log(ultimoUsuario);
            if(ultimoUsuario){
                nuevoUsuario.idUsuario=ultimoUsuario.idUsuario+1;
            }else{
